# Capacitated Network Warehouse Location code - Subsection 6.3 in Technical Appendix  
...
subject to
	...
	#dominance {i in facilities}: 
	#C * y[i] >= sum {j in clients} x[i,j];
	
	# Capacitated version
	satisfyCapacity {i in facilities}: 
	capacity[i] * y[i] >= sum {j in clients} demand[j]*x[i,j];
