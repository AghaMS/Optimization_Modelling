# Capacity cushion code - Subsection 6.5 in Technical Appendix  
...
param capacityCushion{i in facilities}:= 0.9*capacity[i];
...
var kappa{facilities,facilities} >= 0;
...
subject to
	...
	#Either capacity values are preprocessed as above or the below constraint is activated  
	#cushion {i in facilities}: 
	#capacityCushion[i] = 0.85 * capacity[i];
	
	
	# Capacitated version
	satisfyCapacity {i in facilities}: 
	capacityCushion[i] * y[i] = sum {j in clients} (demand[j]*x[i,j]) + delta[i];
